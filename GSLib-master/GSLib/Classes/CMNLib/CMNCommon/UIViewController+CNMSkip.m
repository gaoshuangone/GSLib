...
-(void)setNavigationItem:(void (^)(ButtonItemCoutum* button))initBlock withBlock:(void (^)(TypeItem type) )block{
    ...
    button.butBlock = ^(CNMButton* sender){
        if (block) {
    ...
    if (button.type == Type_Left) {
        self.navigationItem.leftBarButtonItems = [NSArray arrayWithObjects: item,itemSpacer, nil];
    }else{
        self.navigationItem.rightBarButtonItems = [NSArray arrayWithObjects: itemSpacer,item ,nil];
    }
