...
//{
//    return [cnmKit_RegularExpression cnm_Validate_regularIsChinaMobile:phoneNum]? @"中国移动": ([cnmKit_RegularExpression cnm_Validate_regularIsChinaUnicom:phoneNum]? @"中国联通":([cnmKit_RegularExpression cnm_Validate_regularIsChinaTelecom:phoneNum]? @"中国电信": @"未知号码"));
//}
...
{
    NSString *lastNum = [[cnmnkCardlNum substringFromIndex:(cnmnkCardlNum.length-1)] copy];//取出最后一位
    NSString *forwardNum = [[cnmnkCardlNum substringToIndex:(cnmnkCardlNum.length -1)] copy];//前15或18位
...
+ (NSError*)setNSError:( NSError **)error strError:(NSString *)strError {
    if (error != NULL) {
        ...
        *error = [NSError errorWithDomain:NSMachErrorDomain code:CGFLOAT_DEFINED userInfo:userInfo];
        return (*error);
    }
    return nil;
}
